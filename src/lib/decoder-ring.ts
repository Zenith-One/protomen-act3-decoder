export const plaintextArray = 'ACT3ZKWF1XGQEOJLD9BVS57IP206UM4RHNYB'.split('');
...
export function getSolutions(ciphertext: string, targetPlaintext: string = 'A'): string[] {
  let output: string[] = [];
  let cipherArr = ciphertext.replace(/\W+/g,'-').split('-');
  let targetIndex = plaintextArray.indexOf(targetPlaintext);
  let presorted = [];
  for (let p of permutations) {
    let pStr = cipherArr.map(k => p[k] ? p[k] : '#').join('');
    let key = p['ciphertext'][targetIndex];
    let display = `[${targetPlaintext} = ${key}] - ${pStr}`;
    presorted.push({key, display});
  }
  presorted.sort((a,b) => {
  ...
  return presorted.map(i => i.display);
}
